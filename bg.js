...
(() => {
  ...
  // palette: near-black, deep indigo, blue-violet, purple, light violet
  const pal = [[3,8,23],[8,24,90],[18,56,170],[52,110,240],[160,196,255]];
  ...
  function fbm(x, y) {
    return noise(x, y) * .55 + noise(x * 2.1 + 7, y * 2.1 + 3) * .3 + noise(x * 4.3 + 1, y * 4.3 + 9) * .15;
  }
  ...
  function draw(now) {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const wx = fbm(x * s + t * 5, y * s - t * 4) * 2.2,
              wy = fbm(x * s - t * 3 + 9, y * s + t * 6 + 4) * 2.2;
